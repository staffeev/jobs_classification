...
const normalizedName = document.getElementById('normalized-name')
...
async function start() {
	...
	canvas.on('plotly_hover', e => {
		...
		normalizedName.innerText = data.normalized_name[hoverId]
		numSpan.innerText = hoverId
	}).on('plotly_unhover', e => {
		hoverId = -1
		desc.innerText = ''
		name.innerText = ''
		clusterName.innerText = ''
		normalizedName.innerText = ''
		numSpan.innerText = ''
	}).on('plotly_click', _ => {
